const trebuchet = (res) => {
    ...
    const changeStringNumberToNumber = (str) => {
      let output = str;
  
      let matches = [];
  
      numberKeys.forEach((key) => {
        const match = [...output.matchAll(new RegExp(key, "g"))].map(
          (element) => ({ key, index: element.index }),
        );
        matches.push(...match);
      });
      matches = matches.filter(({ index }) => index !== -1);
      matches.sort((a, b) => a.index - b.index);
  
      console.log("before", matches);
  
      matches = matches.filter(({ index, key }, arrayIndex, array) => {
        if (arrayIndex === 0 && index < output.match(/\d/).index) {
          return true;
        }
        return (
          !array[arrayIndex + 1] ||
          array[arrayIndex + 1].index >= key.length + index
        );
      });
    ...
    return res
      ...
      .map((cur) => {
        ...
        let number;
        cur = changeStringNumberToNumber(cur);
        ...
        for (let i = 0; i < cur.length; i++) {
          ...
          if (/\d/.test(element)) {
            number = number ? number[0] + element : element + element;
          }
        }
  ...
  zoneight234
  7pqrstsixteen`;
  // console.log(trebuchet("zoneight234"));
