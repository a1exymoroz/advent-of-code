const gearRatios = (input) => {
    ...
    const getNumber = (index, line) => {
      let number = line[index];
      let left = index;
      let right = index;
  
      while (/\d/.test(line[left - 1]) || /\d/.test(line[right + 1])) {
        const leftElement = line[left - 1];
        if (/\d/.test(leftElement)) {
          number = leftElement + number;
          left = left - 1;
        }
  
        const rightElement = line[right + 1];
        if (/\d/.test(rightElement)) {
          number = number + rightElement;
          right = right + 1;
        }
      }
    ...
    const checkAdjacentNumbers = (rowIndex, colIndex, lines) => {
      let sum = 0;
      let sumNumbers = [];
  
      const startRow = rowIndex === 0 ? rowIndex : rowIndex - 1;
      const stopRow = lines.length === rowIndex ? rowIndex : rowIndex + 1;
      const startCol = colIndex === 0 ? colIndex : colIndex - 1;
      const stopCol = lines.length === colIndex ? colIndex : colIndex + 1;
      for (let row = startRow; row <= stopRow; row++) {
        ...
        for (let col = startCol; col <= stopCol; col++) {
          ...
          if (/\d/.test(element)) {
            const [right, number] = getNumber(col, currentRow);
            // sum += number;
    ...
    for (let row = 0; row < lines.length; row++) {
      ...
      [...line.matchAll(/\*/g)].forEach(({ index }) => {
        result += checkAdjacentNumbers(row, index, lines);
      });
    }
    return result;
  };
  ...
  ...$.*....
  .664.598..`;
  var input = `.......855.442......190..................................969..........520.......59.............................................172..........
