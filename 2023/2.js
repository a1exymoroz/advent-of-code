const conundrum = (input) => {
    ...
    const checkPossibilityGames = (str) => {
      const games = str.split("; ");
  
      for (const game of games) {
        const balls = game.split(", ");
        for (const ball of balls) {
          const [count, key] = ball.split(" ");
          if (map[key] < +count) {
            return false;
          }
        }
    ...
    const powerOfSetOfCubes = (str) => {
      ...
      return Object.values(minMap).reduce((acu, cur) => acu * cur, 1);
    };
  
    let result = 0;
    input.split("\n").forEach((str) => {
      let [id, games] = str.split(": ");
  
      id = id.match(/\d+/)[0];
  
      // if (checkPossibilityGames(games)) {
      //   result += +id;
      // }
  
      result += powerOfSetOfCubes(games);
    });
  ...
  var input = `Game 1: 13 green, 3 red; 4 red, 9 green, 4 blue; 9 green, 10 red, 2 blue
